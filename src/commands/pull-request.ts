...
export class PullRequestCommand extends BaseCommand {
	getHandler(repoApi: RepoApiInterface, taskApi: TasksManagementAPIInterface) {
		return async (args: PRArgs, options: { [p: string]: any }, logger: Logger) => {
			...
			try {
				...
				const issue = await taskApi.getTaskInfo(logger, args.taskId);
				if (issue.status == 'OK' && issue.taskInfo) {
					...
					await Promise.all(packageInfos.map(async (pack) => {
						...
						try {
							const openPR = await repoApi.openPR(logger, pack.repoUrl, branch, 'develop', PRTitle, issue.taskInfo.description);
...
export default new PullRequestCommand();